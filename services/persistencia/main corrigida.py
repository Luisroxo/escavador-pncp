...
import json
import requests
from .db_operations import update_participante_data, update_participante_contato
...
def get_cnpj_data(cnpj_clean: str) -> Optional[dict]:
    ...
    try:
        ...
        data = response.json()
        # Decodificar os valores retornados para UTF-8
        data = {key: safe_decode(value) if isinstance(value, (str, bytes)) else value for key, value in data.items()}
...
def enriquecer_cnpj(cnpj_clean: str):
    ...
    try:
        ...
        rows_affected = update_participante_data(DB_CONFIG, cnpj_clean, data)
...
def atualizar_contato(cnpj_clean: str, whatsapp: str = None, redes_sociais: dict = None):
    ...
    try:
        conn = None  # Inicializar a variável conn
        rows_affected = update_participante_contato(DB_CONFIG, cnpj_clean, whatsapp, redes_sociais)
...
# Adicionar um CNPJ para teste
TEST_CNPJ = "47986478000104"

# Testar o novo CNPJ
if __name__ == "__main__":
    logging.info(f"Testando enriquecimento para o CNPJ: {TEST_CNPJ}")
    try:
        enriquecer_cnpj(TEST_CNPJ)
    except Exception as e:
        logging.error(f"Erro ao testar o CNPJ {TEST_CNPJ}: {e}")
